...
function SafetyDataSheets(props: { history: boolean; logsPage?: boolean }) {
  ...
  const { mutate: deleteMutate } = useMutation(
    DataProviders.SAFETY_DATA_SHEETS.deleteItem,
    {
      onSuccess: () => {
  ...
  const { mutate: updateMutate } = useMutation(
    DataProviders.SAFETY_DATA_SHEETS.updateItem,
    {
      onSuccess: () => {
  ...
  const adminActions = currentUserRoles.includes(ADMIN_ROLE)
    ? (params: { row: SafetyDataSheetInterface }) => [
...

        {
          ...
          handler: () => {
            navigate(
              `/${DataProviders.SAFETY_DATA_SHEETS.endpoint}/` + params.row.id + "/edit",
            );
          },
        },
        {
    ...
    : () => [];
  const actions = (params: { row: SafetyDataSheetInterface }) =>
        ...
        ? [
            {
              ...
              width: 55,
              sortable: false,
              renderCell: (params: { row: SafetyDataSheetInterface }) => (
                <>
      ...
      {DashboardCard({
        title: (
          <Box sx={{ display: "flex", justifyContent: "space-between" }}>
            {props.history ? (
              <>
                {Names.change_history} - {Names.safety_data_sheet}
              </>
            ) : (
              <NewItemButton
                textBefore={Names.safety_data_sheets}
                href={`/${DataProviders.SAFETY_DATA_SHEETS.endpoint}/new`}
              />
            )}
            {!props.history &&
              showHistoryButton(navigate, DataProviders.SAFETY_DATA_SHEETS.endpoint)}
          </Box>
        ),
        content: (
          <>
            <DataGridPagination
              ...
              query_key="getSafetyDataSheets"
              query_fn={
                (props.history
                  ? getLogs(DataProviders.SAFETY_DATA_SHEETS)
                  : DataProviders.SAFETY_DATA_SHEETS
                ).getItemList
              }
